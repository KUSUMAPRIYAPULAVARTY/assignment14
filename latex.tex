...
\begin{align}
  \mathbbf{T}(\vec{A})=\vec{B}\vec{A} \in \vec{W}\\
  \implies \mathbbf{T}_1(\vec{B}\vec{A})=\vec{A} \in \vec{V}\\
  \text{dim}(\vec{V})=mn, \text{dim}(\vec{W})=pn\label{1}
\end{align}
Since $ \mathbbf{T}$ is one-one mapping, the zero vector in $\vec{V}, \vec{0}_{m \times n}$ is uniquely mapped to 
\begin{align}
   \mathbbf{T}(\vec{0}_{m \times n})= \vec{B} \vec{0}_{m \times n}=\vec{0}_{p \times n}\\
     \text{So, }\vec{B}\vec{A}=\vec{0} \iff \vec{A}=\vec{0}\label{3}
\end{align}
...
\begin{align}
    ...
    \iff c_1,c_2,\hdots,c_{mn} \in \mathbbf{F}=0\label{2}
\end{align}
Any matrix $\vec{A} \in \vec{V}$ can be written as
\begin{align}
 \vec{A}= \sum_{i=1}^{mn} \alpha_i\vec{V}_i  
\end{align}
Since $\mathbbf{T}$ is onto, any matrix $\vec{C} \in \vec{W}$ can be expressed as 
\begin{align}
...
\begin{align}
    ...
    \eqref{3} \implies c_1\vec{V}_1+\hdots+ c_{mn}\vec{V}_{mn}=0\\
    \iff c_1,c_2,\hdots,c_{mn}=0 (\text{from \eqref{2}})
\end{align}
So,the set $\vec{S}$ with cardinality $mn$ is basis for $\vec{W}$ 
\begin{align}
   \eqref{1} \implies pn=mn\\
    p=m\label{4}
\end{align}
\eqref{3},\eqref{4} prove that $\vec{B}$ is invertible $m \times m$ matrix.
\subsection{Case 2}
