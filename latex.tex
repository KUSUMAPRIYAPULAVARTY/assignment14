...
\begin{align}
  ...
  \text{dim}(\vec{V})=mn, \text{dim}(\vec{W})=pn\label{6}
\end{align}
...
\end{align}
So, $\cbrak{\vec{B}\vec{V}_1,\vec{B}\vec{V}_2,\hdots,\vec{B}\vec{V}_{mn}}$ forms basis of $\vec{W}$
\begin{align}
   \eqref{6} \implies pn=mn\\
    p=m
\end{align}
Since $ \mathbbf{T}$ is one-one mapping, the zero vector in $\vec{V}, \vec{0}_{m \times n}$ is uniquely mapped to 
\begin{align}
   \mathbbf{T}(\vec{0}_{m \times n})= \vec{B} \vec{0}_{m \times n}=\vec{0}_{m \times n}\\
     \text{So, }\vec{B}\vec{A}=\vec{0} \iff \vec{A}=\vec{0}
\end{align}
This proves that $\vec{B}$ is invertible $m \times m$ matrix.
\subsection{Case 2}
